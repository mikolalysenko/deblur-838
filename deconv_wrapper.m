...
cd release;
fid = fopen('deconv_wrapper_af.txt', 'w+');
...
if( isfield(options, 'nbrRadius') )
    fprintf(fid, '-nbrRadius %d\n', round(options.nbrRadius));
end
if( isfield(options, 'spatialVar') )
    fprintf(fid, '-spatialVar %f\n', options.spatialVar);
end
if( isfield(options, 'intRngVar') )
...
outputImg = imread('deconv_wrapper_output.ppm');
